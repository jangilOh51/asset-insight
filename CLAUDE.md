...
☐ 모든 테스트가 통과하는가?   ← 백엔드 개발 포함 시 반드시 실행 결과 첨부
☐ PLAN.md 상태가 업데이트됐는가?
☐ 영향받은 문서가 업데이트됐는가?
```

> **🔴 테스트 결과서 확인 규칙 (백엔드 개발 시 필수)**
>
> 백엔드 엔드포인트가 추가/변경된 모든 작업에서 PM은 아래를 직접 실행하고 결과를 작업 로그에 기록해야 한다.
>
> ```bash
...
> | 항목 | 기준 |
> |------|------|
> | 전체 통과 여부 | `N passed, 0 failed` 확인 |
> | 신규 엔드포인트 커버 | 성공 / 캐시 / 404 / 유효성 오류 케이스 각 1개 이상 |
> | 기존 회귀 여부 | 기존 통과 케이스가 새 코드로 인해 실패하지 않는지 확인 |
>
> PM이 테스트 결과를 확인하지 않으면 다음 단계(프론트 개발 / 배포)로 진행할 수 없다.
...
### 🔴 작업 로그 규칙 (필수 — 절대 생략 금지)

**모든 작업 담당자는 문서로 소통한다. `docs/work_log/` 폴더가 팀의 유일한 공식 소통 채널이다.**

#### 파일명 규칙
```
docs/work_log/{작업ID}-{YYYYMMDD}.md
예: docs/work_log/S3-01-20260420.md
```
작업 ID가 없는 경우(버그 수정, 설정 변경 등): `misc-{YYYYMMDD}-{짧은설명}.md`

#### 각 역할의 의무

| 역할 | 로그에 반드시 기록해야 할 내용 |
|------|-------------------------------|
| 기획자 | 기능 명세, AC 목록, 범위 결정 근거 |
| UX | 디자인 결정 사항, 5가지 UI 상태 설계 |
| 백엔드 | 구현 내용, 변경 파일 목록, 테스트 결과 |
| 프론트 | 구현 내용, 변경 파일 목록, 5가지 상태 구현 여부 |
| 코드 리뷰어 | MUST/SHOULD/NIT 항목, 수정 요청 사항 |
| QA | 테스트 케이스 목록, 통과/실패 결과 |
| PM | **각 단계 승인 여부 + 승인 이유**, 재작업 지시 내용 |

#### PM 승인 규칙
- PM은 각 역할의 작업 완료 후 반드시 작업 로그를 **직접 읽어서** 내용을 검증한다
- 승인 시 반드시 이유를 기록한다: `✅ 승인 — [이유]`
- 미흡 시 재작업을 지시하고 이유를 기록한다: `🔄 재작업 요청 — [이유] → [기대 결과]`
- PM이 로그를 확인하지 않으면 다음 단계로 진행할 수 없다
